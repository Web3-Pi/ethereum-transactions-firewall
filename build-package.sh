...
# Configuration
PACKAGE_NAME=w3p_tx_firewall
VERSION=${1:-$(git describe --tags --always)}
OUTPUT_DIR="./dist-deb"

echo "Building package for $PACKAGE_NAME version $VERSION"
...
# Create config file
cat > "$OUTPUT_DIR/package/etc/$PACKAGE_NAME/w3p_tx_firewall.conf" << EOF
NODE_ENV=production
SERVER_PORT=8454
PROXY_PORT=18500
WSS_PORT=18501
RPC_ENDPOINT='http://localhost:8545'
AUTHORIZED_ADDR_FN=/etc/$PACKAGE_NAME/auth_addr.json
KNOWN_CONTRACTS_FN=/etc/$PACKAGE_NAME/known_contracts.json
CONTRACT_ABIS_FN=/etc/$PACKAGE_NAME/contract_abis.json
EOF

# Create data files with initial content
cat > "$OUTPUT_DIR/package/etc/$PACKAGE_NAME/auth_addr.json" << EOF
{
...
cat > "$OUTPUT_DIR/package/etc/$PACKAGE_NAME/known_contracts.json" << EOF
{
...
cat > "$OUTPUT_DIR/package/etc/$PACKAGE_NAME/contract_abis.json" << EOF
{
...
[Unit]
Description=Web3 Pi Transaction Firewall Service
After=network.target

[Service]
Type=simple
EnvironmentFile=/etc/$PACKAGE_NAME/w3p_tx_firewall.conf
...
mv package.deb "../w3p-tx-firewall-$VERSION.deb"
cd ..

echo "Package built successfully: w3p-tx-firewall-$VERSION.deb"
